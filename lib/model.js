...
var proto = {
  ...
  __notifyChange: function(key, newValue, oldValue) {
    ...
    this.__ancestors.forEach(function(item) {
      ...
      //item.emit(eventName, new EmitterEvent(eventName, name, target, data));
      //item.emit(eventName + ':' + name, new EmitterEvent(eventName, name, target, data));
      item.emit('change', new EmitterEvent(eventName, path, target, data));
      //item.emit('update' + ':' + name, new EmitterEvent(eventName, name, target, data));
    });
  },
